...
The Agent OS has a provider-neutral execution boundary for property-scoped tools.
...
6. An active explicit agent-property assignment is also required.
7. Consequential tools (`message.queue`, `task.create`, `quote-request.create`, `job.schedule`) require an Agent OS task in `WAITING_APPROVAL` plus a persisted `HUMAN_APPROVAL_GRANTED` event.
8. Consequential execution is recorded as `TOOL_EXECUTED` and completes the approval task after successful execution.
9. Messaging uses the existing idempotency key boundary; live providers remain disabled until provider configuration and compliance onboarding.
