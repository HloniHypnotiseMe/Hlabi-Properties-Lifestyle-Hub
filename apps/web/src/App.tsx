...
import { ArrowRight, Home, Store, UserRound, GraduationCap, ShieldCheck } from 'lucide-react';
...
export default function App() {
  ...
  if (portalOpen) return <HomeownerPortal userId="demo-homeowner" onSignOut={() => setPortalOpen(false)} />;
  ...
  return (
    <main>
      ...
      <section className="hero">
        <div className="shell hero-grid">
          <div>
            ...
            <div className="hero-actions"><a className="primary" href="#hub">Explore your journey <ArrowRight size={18} /></a><button className="secondary" onClick={() => setPortalOpen(true)}>Enter the Lifestyle Hub</button></div>
          ...
          <div className="hero-card" id="renewal">
            <div className="card-label">FEATURED</div><h2>Home Renewal Plan™</h2><p>Turn maintenance from an afterthought into a five-year property plan.</p>
      ...
      <section className="hub shell" id="hub">
        <div className="section-head"><div><p className="eyebrow">BUY · SELL · INVEST</p><h2>Start where you are.</h2></div><p>Each journey begins with a simple intake and expands into the property services, people and tools you need next.</p></div>
        <JourneyExplorer />
        <div className="journey-grid journey-grid-secondary">{secondaryJourneys.map(({ icon: Icon, title, text, cta }) => <article className="journey" key={title}><Icon size={24} /><h3>{title}</h3><p>{text}</p><button onClick={() => title === 'I own a home' && setPortalOpen(true)}>{cta} <ArrowRight size={15} /></button></article>)}</div>
      </section>
