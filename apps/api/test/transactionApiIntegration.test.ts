...
test('transaction API completes accepted-offer to document verification and completion',async()=>{
 const h=await harness();try{
  ...
  const milestone=created.milestones.find((x:any)=>x.key==='DOCUMENTS');assert.ok(milestone);r=await h.request('/api/v1/transactions/'+created.id+'/milestones/'+milestone.id,{method:'POST',role:'SELLER',userId:seller,body:JSON.stringify({status:'IN_PROGRESS',notes:'Documents being reviewed'})});assert.equal(r.status,200);assert.equal((await r.json()).status,'IN_PROGRESS');
