...
export function registerAgentAiRoutes(app: Express, authenticated: RequestHandler, repository: AgentRepository, provider: AiProviderAdapter) {
  app.post('/api/v1/agent-ai/tasks', authenticated, async (req, res) => {
    ...
    if (!principal || principal.role !== 'AGENT') return res.status(403).json({ error: 'ROLE_NOT_ALLOWED' });
    const parsed = schema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: 'INVALID_AI_TASK', details: parsed.error.flatten() });
    const task = await orchestrateAgentTask(repository, provider, { ownerId: principal.userId, ...parsed.data });
    if (!task) return res.status(404).json({ error: 'AGENT_NOT_FOUND_OR_INACTIVE' });
