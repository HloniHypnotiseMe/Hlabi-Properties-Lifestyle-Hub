...
export class PostgresHomeownerRepository implements HomeownerRepository {
  ...
  async getPropertyForOwner(propertyId: string, ownerId: string): Promise<PropertyRecord | null> {
    const result = await this.pool.query(
      `SELECT id, owner_id, nickname, address, property_type, lifecycle
       FROM properties WHERE id = $1 AND owner_id = $2 LIMIT 1`,
      [propertyId, ownerId],
    );
  ...
  async createAudit(input: Omit<AuditRecord, 'id' | 'createdAt' | 'updatedAt'>): Promise<AuditRecord> {
    ...
    try {
      ...
      const auditResult = await client.query(
        `INSERT INTO audits (property_id, owner_id, status)
         VALUES ($1, $2, $3)
         RETURNING id, created_at, updated_at`,
        [input.propertyId, input.ownerId, input.status],
      );
      const row = auditResult.rows[0];
      for (const finding of input.findings) {
        await client.query(
          `INSERT INTO audit_findings
           (audit_id, area, grade, description, priority, recommended_action, verified)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`,
          [finding.area, finding.area, finding.grade, finding.description ?? null, finding.priority, finding.recommendedAction, finding.verified],
        );
      }
  ...
  async getAuditForOwner(auditId: string, ownerId: string): Promise<AuditRecord | null> {
    const result = await this.pool.query(
      `SELECT a.id, a.property_id, a.owner_id, a.status, a.created_at, a.updated_at,
       ...
       FROM audits a
       LEFT JOIN audit_findings f ON f.audit_id = a.id
    ...
    return {
      ...
      findings: result.rows.filter((row) => row.area).map((row) => ({
        area: row.area,
        grade: row.grade,
