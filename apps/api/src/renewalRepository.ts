...
export interface RenewalRepository {
  ...
  getPlanByIdForOwner(planId: string, ownerId: string): Promise<RenewalPlan | null>;
...
export class MemoryRenewalRepository implements RenewalRepository {
 ...
 async getPlanByIdForOwner(planId:string,ownerId:string){return this.plans.find(p=>p.id===planId&&p.ownerId===ownerId)??null;}
...
export class PostgresRenewalRepository implements RenewalRepository {
 ...
 async getPlanByIdForOwner(planId:string,ownerId:string){const r=await this.pool.query('SELECT * FROM renewal_plans WHERE id=$1 AND owner_id=$2',[planId,ownerId]);return r.rows[0]?mapPlan(r.rows[0]):null;}
