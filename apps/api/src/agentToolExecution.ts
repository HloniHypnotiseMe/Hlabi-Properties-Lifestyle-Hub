...
export class AgentToolExecutionService {
  ...
  async execute(ownerId: string, context: AgentToolContext, input: AgentToolExecutionInput): Promise<AgentToolExecutionResult> {
    ...
    switch (input.tool) {
      ...
      case 'job.settle': { const jobId = String(input.arguments.jobId ?? ''); if (!jobId) throw new Error('JOB_ID_REQUIRED'); const job = await this.jobRepository.getJobForOwner(jobId, ownerId); if (!job || job.propertyId !== propertyId) throw new Error('JOB_SCOPE_VIOLATION'); if (!job.quoteId) throw new Error('JOB_QUOTE_REQUIRED'); if (!['REQUESTED','SCHEDULED'].includes(job.status)) throw new Error('JOB_NOT_PAYABLE'); const existing = await this.billingRepository.getSuccessfulTransactionForJob(job.id, ownerId); if (existing) { result = { transaction: existing, alreadySettled: true }; break; } const requestId = typeof job.details.quoteRequestId === 'string' ? job.details.quoteRequestId : undefined; if (!requestId) throw new Error('QUOTE_REQUEST_CONTEXT_REQUIRED'); const quotes = await this.supplierRepository.listQuotesForOwnerRequest(requestId, ownerId); const quote = quotes.find(q => q.id === job.quoteId); if (!quote || quote.status !== 'ACCEPTED' || !quote.amountCents || quote.amountCents <= 0) throw new Error('AUTHORITATIVE_QUOTE_AMOUNT_REQUIRED'); const reference = input.idempotencyKey ?? String(input.arguments.idempotencyKey ?? ('JOB-' + job.id)); await this.agentRepository.updateTask(input.taskId!,ownerId,{status:'RUNNING',startedAt:new Date().toISOString()}); const payment = await this.paymentProvider.createPayment({amountMinor:quote.amountCents,currency:quote.currency,reference,customerId:ownerId}); const transaction = await this.billingRepository.createTransaction({ownerId,propertyId,jobId:job.id,provider:this.paymentProvider.name,reference,providerTransactionRef:payment.externalId,amountMinor:quote.amountCents,currency:quote.currency,status:payment.status,idempotencyKey:reference}); result = { transaction, alreadySettled: false, authoritativeQuoteId: quote.id }; break; }
    }
    if (input.taskId) { const taskStatus = input.tool==='job.settle' && result.transaction && (result.transaction as any).status==='FAILED' ? 'FAILED' : 'COMPLETED'; await this.agentRepository.appendEvent({taskId:input.taskId,ownerId,eventType:'TOOL_EXECUTED',payload:{tool:input.tool,propertyId,consequential,result}}); await this.agentRepository.updateTask(input.taskId,ownerId,{status:taskStatus as any,output:result,errorCode:taskStatus==='FAILED'?'PAYMENT_FAILED':undefined,completedAt:taskStatus==='COMPLETED'?new Date().toISOString():undefined}); }
