...
export class MemoryJobRepository implements JobRepository {
  ...
  private readonly quotes = new Map<string, { id:string; ownerId:string; propertyId:string; supplierId:string; status:string }>();
  ...
  async createJobFromQuote(input:{quoteId:string;ownerId:string;details?:Record<string,unknown>}) { const q=this.quotes.get(input.quoteId); if(!q||q.ownerId!==input.ownerId||!['SUBMITTED','ACCEPTED'].includes(q.status)) return null; const now=new Date().toISOString(); const job:ServiceJob={id:`job-${crypto.randomUUID()}`,propertyId:q.propertyId,ownerId:q.ownerId,supplierId:q.supplierId,quoteId:q.id,status:'REQUESTED',details:input.details??{},createdAt:now,updatedAt:now}; q.status='ACCEPTED'; this.jobs.set(job.id,job); return job; }
